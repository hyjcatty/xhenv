...
function getdevcali(devcode){
    ...
    var getdevcali_callback = function(result){
        var ret = result.status;
        if(ret == "true"){
            for(var k in result.ret){
                console.log("#"+k+"_input");

                console.log(result.ret[k]);
                $("#"+k+"_Input").val(result.ret[k]);
            }
            modal_middle($('#DeviceCalibrationTable'));
            $('#DeviceCalibrationTable').modal('show') ;
        }else {
            show_alarm_module(true, "请重新登录！" + result.msg, null);
        }
    };
    JQ_get(request_head,map,getdevcali_callback);
}
function checkdevcali(){
    var map={
        'dust_coefmax':1,
        'dust_coefmin':1,
        'dust_coefK':1,
    ...
    for(var k in map){
        var temp = $("#"+k+"_Input").val();
        if(temp ===""||isNaN(parseFloat(temp))){
            $("#"+k+"_Input").focus();
            return null;
        }
        map[k]=parseFloat(temp);
    }
    return map;
}
function setdevcali(devcode){
    var calimap = checkdevcali();
    if(calimap === null) return;
    var map={
        action:"SetDevCali",
        body:{
            DevCode:devcode,
    ...
    var setdevcali_callback = function(result){
        var ret = result.status;
        if(ret == "true"){
            $('#DeviceCalibrationTable').modal('hide') ;

            setTimeout(function() {
                show_alarm_module(false, "校准参数修改成功！", null);
            },500);
        }else {
            show_alarm_module(true, "请重新登录！" + result.msg, null);
        }
